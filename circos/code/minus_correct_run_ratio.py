...
# Define the updated karyotype as a multi-line string (unused in this script)
karyotype = """
...
def main():
    ...
    base_graph_dir = "/Users/ahramahn/Library/CloudStorage/OneDrive-UniversityofMiami/1. Research/DMS-MaP-Seq/FASTKD5/FASTKD_project_WT/graph"
    ...
    plus_output_path = os.path.join(base_graph_dir, "WT_plus_ratio_merged_test.txt")
    minus_output_path = os.path.join(base_graph_dir, "WT_minus_ratio_merged_test.txt")
            ...
            if os.path.isfile(csv_minus_path):
                try:
                    ...
                    if {'Position', 'Mutated'}.issubset(df_minus.columns):
                        # Determine the minimum position to adjust positions to start from 1
                        min_pos = df_minus['Position'].min()
                        for _, row in df_minus.iterrows():
                            position = row['Position']
                            adjusted_position = position - min_pos + 1  # Adjust position
                            count = row['Mutated']
