...
def main():
    ...
    plus_output_path = os.path.join(base_graph_dir, "plus_merged.txt")
    minus_output_path = os.path.join(base_graph_dir, "minus_merged.txt")
    ...
    with open(plus_output_path, 'w') as plus_outfile, open(minus_output_path, 'w') as minus_outfile:
        ...
        for gene in gene_sequence:
            ...
            csv_plus_path = os.path.join(gene_plus_dir, 'full', 'profile_related_n-count.csv')
            
            if os.path.isfile(csv_plus_path):
                try:
                    df_plus = pd.read_csv(csv_plus_path)
                    # Check for required columns
                    if {'Position', 'Unambiguous'}.issubset(df_plus.columns):
                        for _, row in df_plus.iterrows():
                            position = row['Position']
                            count = row['Unambiguous']
            ...
            csv_minus_path = os.path.join(gene_minus_path, 'full', 'profile_related_n-count.csv')
            
            if os.path.isfile(csv_minus_path):
                try:
                    df_minus = pd.read_csv(csv_minus_path)
                    # Check for required columns
                    if {'Position', 'Unambiguous'}.issubset(df_minus.columns):
