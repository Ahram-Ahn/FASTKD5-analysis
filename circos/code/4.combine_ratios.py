...
def main():
    # Define the base graph directory
    ...
    plus_output_path = os.path.join(base_graph_dir, "WT_plus_ratio_merged.txt")
    minus_output_path = os.path.join(base_graph_dir, "WT_minus_ratio_merged.txt")
                ...
                try:
                    ...
                    if {'Position', 'Mutated'}.issubset(df_minus.columns):
                        for _, row in df_minus.iterrows():
                            ...
                            count = row['Mutated']
                            # Write: gene, position, position, count
                            minus_outfile.write(f"{gene} {position} {position} {count}\n")
                    else:
